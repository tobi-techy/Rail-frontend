        ...
        <View className="flex-row flex-wrap" style={{ gap: 16 }}>
          {stats.map((s, i) => (
            <View key={i} className="min-w-[90px] flex-1">
              ...
              <Text className="font-heading-bold text-[22px] leading-tight text-text-primary">
                {s.value}
              </Text>
              {s.change && (
                <Text
...
export function TipCard({ card }: { card: InsightCard }) {
  ...
  const severity = (card.data?.severity as string) ?? 'info';
  const bgMap: Record<string, string> = {
    urgent: 'bg-coral-red/10',
    important: 'bg-sunburst-yellow/10',
    info: 'bg-primary/5',
  };
  ...
  return (
    <CardContainer>
      <View
        className={`flex-row items-start gap-3 rounded-2xl p-4 ${bgMap[severity] ?? bgMap.info}`}>
