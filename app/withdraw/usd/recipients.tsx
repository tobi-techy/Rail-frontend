...
export default function UsdRecipientsScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    return recipients.filter(
      (r) => r.accountHolderName.toLowerCase().includes(q) || r.accountNumber.includes(q)
    );
    ...
    <SafeAreaView className="flex-1 bg-warm-canvas" edges={['top']}>
      ...
      <ScrollView
        ...
        <Animated.View entering={FadeInUp.duration(250)}>
          <Text className="font-subtitle text-[28px] text-text-primary">Send USD</Text>
        ...
        <Animated.View entering={FadeInUp.delay(100).duration(250)} className="mt-5">
          <Pressable
            ...
            onPress={() => {
              router.push({
                pathname: '/withdraw/usd/new-recipient' as never,
                params: { amount: params.amount, currency: 'USD' },
              } as never);
            }}>
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((r, i) => (
              <RecipientRow
                ...
                subtitle={`••••${r.accountNumber.slice(-4)} | ${r.routingNumber}`}
                index={i}
