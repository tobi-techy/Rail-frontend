...
export default function EurConfirmScreen() {
  ...
  const sessionKey = `eur-${numericAmount}-${params.iban}`;
  ...
  const executeWithdrawal = useCallback(async () => {
    ...
    const rawIban = (params.iban ?? '').replace(/\s/g, '').toUpperCase();
    if (rawIban.length < 15 || rawIban.length > 34) {
      setErrorMsg('Invalid IBAN length. Please check and try again.');
      return;
    }
    if (!/^[A-Z]{2}\d{2}[A-Z0-9]+$/.test(rawIban)) {
      setErrorMsg(
        'Invalid IBAN format. Must start with a 2-letter country code followed by digits.'
      );
      return;
    }
    ...
    try {
      const idempotencyKey = `eur-${numericAmount}-${rawIban}`;
  ...
  if (status) {
    return (
      <WithdrawalStatusScreen
        ...
        message={
          status === 'success'
            ? 'Your EUR transfer has been sent successfully.'
            : status === 'failed'
              ? errorMsg
              : "Your EUR transfer is being processed. We'll notify you when the funds arrive."
        }
        onDone={() => {
      ...
      <ScrollView
        ...
        <Animated.View entering={FadeInUp.delay(STAGGER_MS).duration(250)}>
          ...
          <DetailCard>
            ...
            <Hairline />
            <DetailField label="Currency" value="EUR" />
          </DetailCard>
        </Animated.View>
